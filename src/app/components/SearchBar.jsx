    ...
    <div className="relative" ref={searchRef}>
      <form onSubmit={handleSubmit} className="flex">
        <input
          ...
          className="w-full px-4 py-2 rounded-l bg-gray-700 focus:outline-none focus:ring-2 focus:ring-yellow-400 text-white"
        />
        <button
          type="submit"
          className="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-r focus:outline-none transition-colors"
        >
        ...
        <div className="absolute top-full left-0 right-0 mt-1 bg-gray-800 rounded-lg shadow-lg overflow-hidden z-50">
          {searchResults.map((result) => (
            <Link
              key={result.id}
              href={result.path}
              className="flex items-center px-4 py-3 hover:bg-gray-700 transition-colors border-b border-gray-700 last:border-0"
              onClick={() => {
                setIsSearching(false);
                setSearchQuery('');
              }}
